...
function rebuildCart(){
	...
	if(mfp.Vc.length > 0){
		for(var i=0;i<mfp.Vc.length;i++){
			...
			var taxPar = "";
			if(mfp.Vc[i]['tax']){
				tax = subtotal * (Number(mfp.Vc[i]['tax']) / 100);
				taxTotal += tax;
				taxPar = "[ " + mfp.Vc[i]['tax'] + "% ] ";
			};
			var className = 'mfp_colored';
			if(i % 2 == 0) className = 'mfp_achroma';
			cartHTML += '<tr class="'+className+'">';
			cartHTML += '<td></td>'; // image
			cartHTML += '<th>'+mfp.Vc[i]['name']+'<span>( '+mfp.Vc[i]['id']+' / '+mfpStr(mfp.cm(mfp.Vc[i]['price']))+' )</span></th>';
			cartHTML += '<td><select onchange="updateCart(this,'+i+')">'+createOption(mfp.Vc[i]['qty'])+'</select></td>';
			cartHTML += '<td class="msc_price">' + taxPar + mfpStr(mfp.cm(tax)) + '</td>';
			cartHTML += '<td class="msc_price">'+mfpStr(mfp.cm(subtotal))+'</td>';
			cartHTML += '</tr>';
		...
		renderHTML += '<td>'+mfpLang['cart']['tax']+'</td>';
		...
		renderHTML += '<tfoot>';
		renderHTML += '<tr>';
		renderHTML += '<td colspan="3">&nbsp;</td>';
		//renderHTML += '<td>&nbsp;</td>';
		renderHTML += '<td class="msc_price">'+mfpStr(mfp.cm(taxTotal))+'</td>';
...
mfp.extend.event('calc',
	function(){
		for(var i=0;i<mfp.Vc.length;i++){
			...
			var price = (Number(mfp.Vc[i]['price']) * Number(mfp.Vc[i]['qty']))
			mfp.Price += price;
			var tax = mfp.Vc[i]['tax'];
			if(tax){
				if(!mfp.Tax[tax]){
					mfp.Tax[tax] = 0;
					mfp.NetPrice[tax] = 0;
