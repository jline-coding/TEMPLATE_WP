function MailformPro(){
	...
	this.calc = function(){
		if(mfp.$('mfp_price')){
			...
			for(var prop in mfp.Items){
				var qty = 1;
				var tax = mfp.$(prop).getAttribute('data-tax') || 0;
				if(!mfp.Tax[tax]){
					mfp.Tax[tax] = 0;
					mfp.NetPrice[tax] = 0;
				...
				if(mfp.$(prop).tagName.toLowerCase() == 'option' && mfp.$(prop).selected && !mfp.$(prop).disabled && !mfp.$(prop).parentNode.disabled && !mfp.$(prop).parentNode.parentNode.disabled){
					...
					if(mfp.$(prop).getAttribute('data-net-price')){
						mfp.NetPrice[tax] += parseInt(mfp.$(prop).getAttribute('data-net-price')) * qty
						mfp.GrossPrice[tax] += mfp.Items[prop].price * qty;
					};
					mfp.Tax[tax] += mfp.Items[prop].price * qty;
					mfp.Qty += qty;
					...
					if(mfp.$(prop).getAttribute('data-net-price')){
						mfp.NetPrice[tax] += parseInt(mfp.$(prop).getAttribute('data-net-price')) * Number(mfp.$(prop).value) * qty;
						mfp.GrossPrice[tax] += (mfp.Items[prop].price * Number(mfp.$(prop).value)) * qty;
					};
					mfp.Tax[tax] += (mfp.Items[prop].price * Number(mfp.$(prop).value)) * qty;
					mfp.Qty += (Number(mfp.$(prop).value) * qty);
					if(mfp.$(prop).getAttribute('data-cost')){
			...
			mfp.extend.run('calc');
			var taxTotal = 0;
			for(var prop in mfp.Tax){
				var objects = mfp.byClassName(mfp.Mfp,'mfp_tax_'+prop);
				var zeiritsu = parseInt(prop);
				var zeinuki = parseInt(mfp.Tax[prop] / (zeiritsu / 100 + 1));
				var tax = mfp.Tax[prop] - zeinuki;
				if(mfp.NetPrice[prop] && mfp.GrossPrice[prop]){
					tax = mfp.GrossPrice[prop] - mfp.NetPrice[prop];
				};
				taxTotal += tax;
				for(var i=0;i<objects.length;i++){
					if(objects[i].tagName == "INPUT"){
						objects[i].value = mfpLang['PostPrice'].replace('$1',mfp.cm(tax));
					}
					else {
						mfp.count(objects[i],10,tax,mfpLang['Price']+mfp.CartText);
					};
				};
			};
			(function(){
				var objects = mfp.byClassName(mfp.Mfp,'mfp_tax_net');
				for(var i=0;i<objects.length;i++){
					if(objects[i].tagName == "INPUT"){
						objects[i].value = mfpLang['PostPrice'].replace('$1',mfp.cm(mfp.Price-taxTotal));
					}
					else {
						mfp.count(objects[i],10,mfp.Price-taxTotal,mfpLang['Price']+mfp.CartText);
					};
				};
			})();
			...
			mfp.count(mfp.$('mfp_price'),10,mfp.Price,mfpLang['Price']+mfp.CartText);
			
			//
			var objects = mfp.byClassName(mfp.Mfp,'mfp_price');
			for(var i=0;i<objects.length;i++){
				mfp.count(objects[i],10,mfp.Price,mfpLang['Price']+mfp.CartText);
			};
			//
			var objects = mfp.byClassName(mfp.Mfp,'mfp_qty');
			for(var i=0;i<objects.length;i++){
				mfp.count(objects[i],10,mfp.Qty,'$1');
			};
		...
		for(var i=0;i<mfp.Names.length;i++){
			...
			if(e.name.match(/^mfp_h_/)){
				...
				if(!ElementsFlag[mfp.Names[i]]){
					...
					if(val != "" && mfp.Names[i].indexOf('mfp_') == -1 && !e.getAttribute('data-post-disable') && !e.getAttribute('data-confirm-disable')){
						var className = 'mfp_colored';
						if(s % 2 == 0){
							className = 'mfp_achroma';
						};
	...
	this.count = function(obj,step,endpoint,format){
		var form = format || '$1';
		if(!mfp.countInterval){
			mfp.countInterval = [];
		};
		if(!obj.id){
			obj.id = 'mfp_' + parseInt(Math.random() * 100000);
		};
		clearTimeout(mfp.countInterval[obj.id]);
		var point = obj.innerHTML;
		...
		if(endpoint == point || step < 1){
			obj.innerHTML = form.replace('$1',mfp.cm(endpoint));
		}
		else {
			var showPoint;
			if(endpoint > point){
				showPoint = parseInt(point + ((endpoint - point) / step));
			}
			else {
				showPoint = parseInt(point - ((point - endpoint) / step));
			};
			obj.innerHTML = form.replace('$1',mfp.cm(showPoint));
			step--;
			mfp.countInterval[obj.id] = setTimeout(function(){
				mfp.count(obj,step,endpoint,format);
			},50);
		};
	};
	this.startup = function(){
		...
		if(location.search){
			...
			for(var i=0;i<gets.length;i++){
				var get = new Array();
				get = gets[i].split('=');
				this.GET[decodeURIComponent(get[0])] = decodeURIComponent(get[1]).replace(/\+/ig,' ');
			}
		}
