...
mfp.extend.event('init',
	function(obj){
		if(obj.getAttribute('data-daystart') && obj.getAttribute('data-daymax')){
			...
			var limithour = obj.getAttribute('data-limithour');
			...
			if(limithour){
				...
				var tde = new Date();
				var ty = tde.getFullYear();
				var tm = tde.getMonth() + 1;
				var td = tde.getDate();
				var th = tde.getHours();
				var tmin = tde.getMinutes();
				var limit = new Date(ty+'/'+tm+'/'+td+' '+limithour);
				if(tde.getTime() > limit.getTime()){
					daystart++;
				};
			};
			//
