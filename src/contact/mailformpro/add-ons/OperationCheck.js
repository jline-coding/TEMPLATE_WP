...
mfp.extend.event('ready',
	function(){
		var version = 'MailformPro 4.3.5 - 2026/01/03';
		...
		innerHTML += '<p>'+version+' は正常に動作しています。 <button type="button" onclick="mfpOperationCheckDetail(this)">[ 詳細を表示する ]</button></p>';
...
mfp.extend.event('init',
	function(e){
		if(e.name && !mfpElementsListCheck[e.name] && !e.name.match(/mfp_/)){
			if(e.type != 'checkbox'){
				mfpElementsList.push(e.name);
				mfpElementsListCheck[e.name] = true;
			}
			else {
				mfpElementsList.push(e.name + '_' + e.value);
			};
		}
	}
);
